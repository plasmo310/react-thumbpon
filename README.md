...
Illustrator / Photoshop / Figma を毎回開かずに、決まったレイアウトの画像をさっと作ることを目的にしています。<br>
自由度よりも、速さ・軽さ・操作の少なさを優先しています。

画像は**サーバーに送信されません**。<br>
読み込み・編集・保存・書き出しはすべてブラウザ内で完結します。
...
左上の「サムネイル」パネルで、編集するサムネイルを選びます。<br>
必要なら `＋` で追加し、選択中の行からキャンバスサイズを `1920 × 1080` または `800 × 600` に変更します。

#### 2. 画像を追加する

画像ファイルをキャンバスへ直接ドラッグ&ドロップすると、その位置に画像レイヤーが追加されます。<br>
素材パネルへ先に登録した画像は、クリックで中央へ配置、またはドラッグで任意の位置へ配置できます。

#### 3. テキストを追加する

「レイヤー」パネルの `＋ テキスト` を押します。<br>
追加されたテキストを選ぶと、行の下にプロパティが開き、内容・フォント・サイズ・色などを編集できます。

#### 4. 配置と見た目を整える

キャンバス上でレイヤーをドラッグして動かします。<br>
選択枠のハンドルで大きさや回転を調整し、レイヤー一覧をドラッグして前後関係を変更します。<br>
背景は一覧最上部の `BG 背景` から編集します。

### 画像の書き出し

PNGファイルとしての書き出しをサポートしています。<br>
